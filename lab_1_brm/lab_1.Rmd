...
top_5_status <- sf_trees %>% 
  ...
  rename(tree_count = n) %>% 
  relocate(tree_count) %>% #moves tree_count column to the left
  slice_max(tree_count, n = 5) %>% #takes top 5
...
shift-cmd-c to commment/uncomment line quickly
```{r}
# sf_trees$legal_status %>% unique()
# unique(sf_trees$caretaker)
permitted_data_df <- sf_trees %>% 
  filter(legal_status %in% c('Permitted Site', 'Private') & caretaker %in% 'MTA') # can use comma or & which means both legal_status and caretaker need to match to filter
```

** Example 3:** Only keep Blackwood Acacia trees, and then only keep columns `legal_status`, `date`, `latitutde`, `longitude` and store as `blackwood_acacia_df`

```{r}
blackwood_acacia_df <- sf_trees %>%
  filter(str_detect(species, 'Blackwood Acacia')) %>% # detects words within cells so you don't have to type full scientific name
...
```{r}
# separate items separated by :: into 2 columns
